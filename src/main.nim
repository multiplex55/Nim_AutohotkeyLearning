...
proc setupHotkeys(configPath: string): bool =
  ...
  for hk in config.hotkeys:
    ...
    try:
      ...
      logger.info("Registered hotkey", [("keys", hk.keys), ("action", hk.action)])
    except IOError as e:
      logger.error("Failed to register hotkey", [("keys", hk.keys), ("error", e.msg)])
...
when isMainModule:
  let configPath = if paramCount() >= 1: paramStr(1) else: DEFAULT_CONFIG
